...
import com.yigu.house.widget.CityDialog;
...
public class EditAddrActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        province = mapiAddrResult.getProvince();
        city = mapiAddrResult.getCity();
        county = mapiAddrResult.getArea();

        provinceID = mapiAddrResult.getProvince_id();
        cityID = mapiAddrResult.getCity_id();
        countyID = mapiAddrResult.getArea_id();

        name.setText(mapiAddrResult.getName());
        mobile.setText(mapiAddrResult.getMobile());
        ...
        if(mapiAddrResult.getIs_default().equals("0")) {
            isDefault = 0;
            defaultIv.setImageResource(R.mipmap.sel_rect_black);
        }
        else if(mapiAddrResult.getIs_default().equals("1")) {
            isDefault = 1;
            defaultIv.setImageResource(R.mipmap.sel_rect_orange);
        }
    }

    private void load(){
        showLoading();
        CommonApi.allcity(this, new RequestCallback<JSONObject>() {
            @Override
    ...
    private void modifyAddr(){
        ...
        CommonApi.editAddress(this,mapiAddrResult.getAddress_id(),consignee, tel, provinceID, cityID, countyID, addressStr,isDefault+"", new RequestCallback() {
            @Override
    ...
    @OnClick({R.id.back, R.id.area_ll,R.id.tv_right,R.id.defaultIv})
