package com.yigu.house.activity.vip;
...
import com.yigu.house.fragment.vip.VipInfoFragment;
...
public class VipDetailActivity extends BaseActivity {
    ...
    private VipInfoFragment topFragment;
    ...
    private void initView() {
        ...
        topFragment = new VipInfoFragment();
        bottomFragment = new PromptDetailFragment();

        topFragment.setSizeOpenListener(new VipInfoFragment.SizeOpenListener() {
            @Override
            public void open() {
                album();
            }
...
//                ((PromptDetailFragment) bottomFragment).load();
    ...
    @OnClick({R.id.back, R.id.iv_right_two,R.id.fab,R.id.purcase,R.id.order,R.id.swap})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.swap://申请互调
                ...
                if(checkVip()){
                    String attrId3 = sizeColorLayout.getAttrIds();
                    if(TextUtils.isEmpty(attrId3)){
                        MainToast.showShortToast("请选择商品");
                        return;
                    }
                    DebugLog.i("attrIds:"+attrId3);
                    DebugLog.i("attrIds:"+attrId3);
                    showLoading();
                    ItemApi.buynow(this, goods_id, attrId3,"1", new RequestCallback<List<MapiCarResult>>() {
                        @Override
                        public void success(List<MapiCarResult> success) {
                            hideLoading();
